...
* Задачи разбиты на модули, которые должны быть поддержаны в структуре проекта. То есть каждый из этих модулей должен соответсвовать gradle модулю.
* У звукого и конфигурационного модуля должен быть строго занный интерфейс. Каждой модели, также должен соответсвовать свой неизменяемый интерфейс.
* Любое изменение в интерфейсе требует обсуждения.
* Дерево зависимости модулей:
```
	- core
		- model
		- config
			...
			- model
```
Это значит, что например алгоритмы из config не могут использовать функции из core и тп.
* Тег `UI` значит, что отмеченная реализация может использовать UI для получения информации от пользователя. 
